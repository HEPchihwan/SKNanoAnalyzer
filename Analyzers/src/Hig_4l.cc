...
void Hig_4l::initializeAnalyzer() {
    cout << "[Hig_4l::initializeAnalyzer] Starting initialization" << endl;
    
    // Check user flags
    RunSyst = HasFlag("RunSyst");
    
    cout << "[Hig_4l::initializeAnalyzer] RunSyst = " << RunSyst << endl;
    ...
    if (DataEra == "2016preVFP" || DataEra == "2016postVFP" || DataEra == "2018") {
        ...
        cerr << "[Hig_4l::initializeAnalyzer] DataEra is not set properly: " << DataEra << endl;
        exit(EXIT_FAILURE);
    }
    
    cout << "[Hig_4l::initializeAnalyzer] IsoMuTriggerName = " << IsoMuTriggerName << endl;
    cout << "[Hig_4l::initializeAnalyzer] TriggerSafePtCut = " << TriggerSafePtCut << endl;
    ...
    cout << "[Hig_4l::initializeAnalyzer] Initialization complete" << endl;
}

void Hig_4l::executeEvent() {
...
void Hig_4l::executeEventFromParameter() {
...
RVec<Muon> Hig_4l::SelectMuons(const RVec<Muon>& muons) {
...
RVec<Jet> Hig_4l::SelectJetsFromTTbar(const RVec<Jet>& jets) {
...
RVec<Jet> Hig_4l::SelectBTaggedJets(const RVec<Jet>& jets) {
...
RVec<Jet> Hig_4l::RemoveOverlapWithMuons(const RVec<Jet>& jets, const RVec<Muon>& muons, float deltaR_cut) {
...
RVec<Jet> Hig_4l::RemoveOverlapWithJets(const RVec<Jet>& jets, const RVec<Jet>& bjets, float deltaR_cut) {
    RVec<Jet> cleaned_jets;
    for (const auto& jet : jets) {
        bool overlaps = false;
...
bool Hig_4l::PassEventSelection(const RVec<Muon>& muons, const RVec<Jet>& bjets, const RVec<Jet>& lightjets) {
    return (muons.size() >= 1 && bjets.size() >= 2 && lightjets.size() >= 2);
}

Hig_4l::TTbarObservables Hig_4l::CalculateTTbarObservables(const Muon& lepton,
...
float Hig_4l::CalculateTTbarSystemTransverseMass(const Muon& lepton,
                                                    const Jet& b1, const Jet& b2,
                                                    const Jet& j1, const Jet& j2,
                                                    float met_pt, float met_phi) {
...
float Hig_4l::CalculateTTbarSystemTransverseMassV2(const Muon& lepton,
                                                      const Jet& b1, const Jet& b2,
                                                      const Jet& j1, const Jet& j2,
                                                      float met_pt, float met_phi) {
