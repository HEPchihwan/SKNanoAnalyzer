...
class SignalBackgroundCanvas():
    ...
    def draw(self):
        ...
        if self.background_stack:
            ...
            if stack_hist:
                # Set x-axis range to config values and limit bins
                stack_hist.GetXaxis().SetRangeUser(self.xmin, self.xmax)
                # Force axis limits
                bin_min = stack_hist.FindBin(self.xmin)
                bin_max = stack_hist.FindBin(self.xmax)
                stack_hist.GetXaxis().SetRange(bin_min, bin_max)
        ...
        if self.data_hist:
            # Set x-axis range for data histogram and limit bins
            self.data_hist.GetXaxis().SetRangeUser(self.xmin, self.xmax)
            # Force axis limits for data
            bin_min = self.data_hist.FindBin(self.xmin)
            bin_max = self.data_hist.FindBin(self.xmax)
            self.data_hist.GetXaxis().SetRange(bin_min, bin_max)
            if not first_drawn:
        ...
        if self.ratio:
            # Create dummy histogram for ratio pad axis using the same binning as the actual histograms
            if self.total_background:
...
def rebin_to_common_binning(hist, xmin=0, xmax=2000, nbins=200):
...
def load_histogram(file_path, hist_name, systematic="Central", silent=False):
    ...
    # Apply common binning to avoid merge issues
    rebinned_hist = rebin_to_common_binning(hist_clone, xmin=0, xmax=2000, nbins=200)
...
def plot_signal_background_comparison(data_dir, hist_name, config, output_name="signal_background_comparison", systematic="Central"):
    ...
    # Load background histograms (all MC files)
...
def main():
    ...
    plot_config = {
        "xRange": [0, 2000],        # Extended range to show TTLJ signal at ~2000 GeV
        "yRange": [0.5, 1.5],       # Ratio plot range
        "xTitle": "m_lljj [GeV]",
        "yTitle": "Events / bin",
        "logy": True,               # Log scale
        "rebin": 10,                 # Rebin factor
        "ymax": 10,             # Higher max for log scale
        "ymin": 1e-2                 # Small positive value for log scale
    }
    
    # Create the plot
    hist_name = "WRMass_Central"
    output_name = "WR_CR_2022"
